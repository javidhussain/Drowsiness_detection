...
model = load_model('model/Drowsiness.h5')
path = os.getcwd()
...
while(True):
    ...
    if ret:
        ...
        for (x,y,w,h) in right_eye:
            ...
            r_eye = cv2.resize(r_eye,(75,75))
            r_eye= r_eye/255
            r_eye=  r_eye.reshape(75,75,-1)
            r_eye = np.expand_dims(r_eye,axis=0)
            [rpred] = model.predict(r_eye)
            # print('Right Eye:-',rpred)
            if(rpred[0] > 0.5):
                lbl='Open' 
            if(rpred[0] < 0.5):
                lbl='Closed'
        ...
        for (x,y,w,h) in left_eye:
            ...
            l_eye = cv2.resize(l_eye,(75,75))
            l_eye= l_eye/255
            l_eye=l_eye.reshape(75,75,-1)
            l_eye = np.expand_dims(l_eye,axis=0)
            [lpred] = model.predict(l_eye)
            # print ('Left Eye:-',lpred)
            if(lpred[0] > 0.5):
                lbl='Open'   
            if(lpred < 0.5):
                lbl='Closed'
